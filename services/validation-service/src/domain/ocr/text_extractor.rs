...
impl TextExtractor {
    ...
    pub fn extract_structured_data(&self, text: &str, document_type: &str) -> AlbergueResult<std::collections::HashMap<String, String>> {
    ...
    fn extract_passport_fields(&self, text: &str, data: &mut std::collections::HashMap<String, String>) {
        use regex::Regex;
        ...
        // Extract expiry date
        if let Ok(expiry_regex) = Regex::new(r"(?i)(?:date of expiry|fecha de caducidad)[:\s]+(\d{2}/\d{2}/\d{4})") {
