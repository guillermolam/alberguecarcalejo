...
pub fn init() {
    ...
    if let Ok(storage) = get_storage() {
        ...
        if storage.get_item(SESSIONS_STORAGE_KEY).unwrap_or(None).is_none() {
        ...
        if storage.get_item(CREDENTIALS_STORAGE_KEY).unwrap_or(None).is_none() {
...
pub fn change_admin_credentials(current_password: &str, new_username: &str, new_password: &str) -> Result<(), String> {
    let storage = get_storage().map_err(|_| "Storage unavailable")?;
