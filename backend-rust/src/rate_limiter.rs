...
impl RateLimiter {
    pub fn new() -> Self {
        ...
        configs.insert("DOCUMENT_VALIDATION".to_string(), RateLimitConfig {
            max_requests: 10,
        ...
        configs.insert("OCR_PROCESSING".to_string(), RateLimitConfig {
            max_requests: 5,
            window_ms: 10 * 60 * 1000, // 10 minutes
        });
        
        configs.insert("EMAIL_VALIDATION".to_string(), RateLimitConfig {
            max_requests: 20,
            window_ms: 60 * 60 * 1000, // 1 hour
        });
        
        configs.insert("PHONE_VALIDATION".to_string(), RateLimitConfig {
            max_requests: 20,
            window_ms: 60 * 60 * 1000, // 1 hour
        });
        
        configs.insert("ADMIN_AUTH".to_string(), RateLimitConfig {
            max_requests: 5,
            window_ms: 60 * 60 * 1000, // 1 hour
        });
        
        configs.insert("ADMIN_OPERATIONS".to_string(), RateLimitConfig {
            max_requests: 50,
            window_ms: 60 * 60 * 1000, // 1 hour
        });
        ...
        match existing {
            Some(limit) if now < limit.reset_time => {
                if limit.count >= config.max_requests {
                    ...
                    // Increment count
                    self.limits.insert(key, RateLimit {
                        count: limit.count + 1,
