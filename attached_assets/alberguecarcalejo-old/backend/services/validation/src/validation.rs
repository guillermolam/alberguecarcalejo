...
impl ValidationService {
    ...
    pub async fn validate_document(
        ...
        if !rate_limiter
            .check_limit(&client_id, "DOCUMENT_VALIDATION")
            .await
        {
        ...
        let validation_result =
            self.validate_document_number(&body.document_type, &body.document_number);
    ...
    pub async fn validate_email(
        ...
        if !rate_limiter
            .check_limit(&client_id, "EMAIL_VALIDATION")
            .await
        {
            return Response::from_json(&json!({
                "success": false,
        ...
        Response::from_json(&ValidationResponse {
            ...
            data: ValidationData {
                valid: is_valid,
                message: if is_valid {
                    Some("Valid email format".to_string())
                } else {
    ...
    pub async fn validate_phone(
        ...
        if !rate_limiter
            .check_limit(&client_id, "PHONE_VALIDATION")
            .await
        {
            return Response::from_json(&json!({
                "success": false,
        ...
        Response::from_json(&ValidationResponse {
            ...
            data: ValidationData {
                valid: is_valid,
                message: if is_valid {
                    Some("Valid phone number".to_string())
                } else {
    ...
    fn validate_document_number(
        ...
        match document_type.to_uppercase().as_str() {
            ...
            _ => DocumentValidationResult {
                valid: false,
                message: "Invalid document type".to_string(),
            },
        }
    }
    ...
    fn validate_spanish_dni(&self, dni: &str) -> DocumentValidationResult {
        ...
        if !dni_regex.is_match(dni) {
            return DocumentValidationResult {
                valid: false,
                message: "DNI format is invalid".to_string(),
            };
        }
        ...
        let expected_letter = valid_letters
            .chars()
            .nth((numbers.parse::<usize>().unwrap()) % 23)
            .unwrap();

        if letter != expected_letter {
            return DocumentValidationResult {
                valid: false,
                message: "DNI check digit is invalid".to_string(),
            };
        }

        DocumentValidationResult {
            valid: true,
            message: "DNI is valid".to_string(),
        }
    }
    ...
    fn validate_spanish_nie(&self, nie: &str) -> DocumentValidationResult {
        ...
        if !nie_regex.is_match(nie) {
            return DocumentValidationResult {
                valid: false,
                message: "NIE format is invalid".to_string(),
            };
        }
        ...
        match first_letter {
            ...
            _ => {
                return DocumentValidationResult {
                    valid: false,
                    message: "Invalid NIE prefix".to_string(),
                }
            }
        }
        ...
        if letter != expected_letter {
            return DocumentValidationResult {
                valid: false,
                message: "NIE check digit is invalid".to_string(),
            };
        }

        DocumentValidationResult {
            valid: true,
            message: "NIE is valid".to_string(),
        }
    }
    ...
    fn validate_passport(&self, passport: &str) -> DocumentValidationResult {
        ...
        if !passport_regex.is_match(passport) {
            return DocumentValidationResult {
                valid: false,
                message: "Passport format is invalid".to_string(),
            };
        }

        DocumentValidationResult {
            valid: true,
            message: "Passport format is valid".to_string(),
        }
    }
