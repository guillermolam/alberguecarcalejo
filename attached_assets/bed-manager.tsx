...
import { BedRoomGroup } from "./bed-room-group";
import { BedStatusModal } from "./bed-status-modal";
...
export function BedManager() {
  ...
  // removed getBedColor - now handled in BedButton = (status: string) => {
    switch (status) {
      case 'available':
  ...
  // removed getStatusDot - now handled in BedButton = (status: string) => {
    switch (status) {
      case 'available':
        return 'bg-green-500';
      case 'occupied':
    ...
    <div className="space-y-6">
      ...
      {selectedBed && (
        <BedStatusModal
          ...
          isPending={updateBedStatusMutation.isPending}
        />
      )}
    </div>
  );
