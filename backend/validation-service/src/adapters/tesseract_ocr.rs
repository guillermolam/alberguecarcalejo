...
use shared::{AlbergueResult, AlbergueError};
...
impl OCRClient for TesseractOCR {
    async fn extract_text(&self, image_data: &[u8]) -> AlbergueResult<String> {
        ...
        wasm_bindgen_futures::JsFuture::from(
            js_sys::Promise::resolve(&wasm_bindgen::JsValue::from(42))
        ).await.map_err(|_| AlbergueError::OCRProcessing {
    ...
    async fn extract_text_with_confidence(&self, image_data: &[u8]) -> AlbergueResult<(String, f32)> {
