...
impl PassportValidator {
    ...
    pub fn extract_passport_data(&self, passport_text: &str) -> AlbergueResult<ExtractedData> {
        ...
        let surname_regex = Regex::new(r"(?i)(?:surname|apellidos?)[:\s]+([A-ZÁÉÍÓÚÑÜ\s]+)").unwrap();
        if let Some(captures) = surname_regex.captures(passport_text) {
            extracted.surname = Some(captures[1].trim().to_string());
        }

        // Extract birth date
        let date_regex = Regex::new(r"(?i)(?:date of birth|fecha de nacimiento)[:\s]+(\d{2})[/.-](\d{2})[/.-](\d{4})").unwrap();
        ...
        // Extract nationality
        let nationality_regex = Regex::new(r"(?i)(?:nationality|nacionalidad)[:\s]+([A-ZÁÉÍÓÚÑÜ\s]+)").unwrap();
        if let Some(captures) = nationality_regex.captures(passport_text) {
            extracted.nationality = Some(captures[1].trim().to_string());
        }

        // Extract expiry date
        let expiry_regex = Regex::new(r"(?i)(?:date of expiry|fecha de caducidad)[:\s]+(\d{2})[/.-](\d{2})[/.-](\d{4})").unwrap();
    ...
    fn extract_mrz_data(&self, text: &str, extracted: &mut ExtractedData) -> AlbergueResult<()> {
        ...
        let mrz_line2_regex = Regex::new(r"([A-Z0-9<]{9})<([A-Z]{3})<(\d{6})<([MF])<(\d{6})<").unwrap();
