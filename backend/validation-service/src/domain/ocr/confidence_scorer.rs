...
impl ConfidenceScorer {
    ...
    pub fn calculate_field_confidence(&self, field_name: &str, extracted_value: &str, context: &str) -> f32 {
        match field_name {
    ...
    fn score_name_field(&self, value: &str) -> f32 {
        ...
        let letter_ratio = cleaned.chars().filter(|c| c.is_alphabetic()).count() as f32 / cleaned.len() as f32;
        
        if letter_ratio > 0.8 {
            // Additional checks for Spanish name patterns
            if cleaned.chars().all(|c| c.is_alphabetic() || c == ' ' || "ÁÉÍÓÚÑÜ".contains(c)) {
                return 0.9;
            }
            return 0.7;
        }
    ...
    fn score_nationality_field(&self, value: &str) -> f32 {
        ...
        let common_nationalities = [
            "ESPAÑOLA", "ESPAÑOLA", "FRANCESA", "ALEMANA", "ITALIANA", "PORTUGUESA",
            "BRITÁNICA", "IRLANDESA", "HOLANDESA", "BELGA", "SUIZA", "AUSTRIACA",
            "POLACA", "CHECA", "HÚNGARA", "AMERICANA", "CANADIENSE", "AUSTRALIANA",
            "JAPONESA", "COREANA", "BRASILEÑA", "ARGENTINA", "CHILENA", "MEXICANA",
        ];
        ...
        if cleaned.len() >= 4 && cleaned.len() <= 20 && 
    ...
    pub fn suggest_improvements(&self, field_scores: &HashMap<String, f32>) -> Vec<String> {
        ...
        for (field, score) in field_scores {
            if *score < 0.5 {
                match field.as_str() {
                    "document_number" => suggestions.push("Document number may be unclear - try better lighting".to_string()),
                    "name" | "surname" => suggestions.push(format!("{} field may need manual verification", field)),
                    "birth_date" | "expiry_date" => suggestions.push(format!("{} format unclear - verify manually", field)),
